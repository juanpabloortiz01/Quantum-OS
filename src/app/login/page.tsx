  ...
  if (status === "loading" || status === "authenticated") {
    return (
      <div className="min-h-screen bg-[#FBFBFA] flex items-center justify-center">
        <span className="text-[#1A1A1A] text-sm font-semibold animate-pulse">Cargando...</span>
      </div>
    )
  }

  return (
    <div className="min-h-screen bg-[#FBFBFA] text-[#1A1A1A] flex items-center justify-center p-4 selection:bg-slate-200 selection:text-black font-sans relative">

      {/* DOT GRID igual que el onboarding */}
      <div
      ...
      <div className="relative z-10 w-full max-w-md">
        {/* TARJETA — mismo estilo que el onboarding */}
        <motion.div
          initial={{ opacity: 0, y: 16 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, ease: [0.25, 0.1, 0.25, 1] }}
          className="border border-[#E2E8F0] bg-white rounded-xl shadow-sm relative overflow-hidden"
        >
          {/* Franja naranja superior igual a la barra de progreso del onboarding */}
          <div className="h-1 bg-[#F54927]" />

          {/* Contenido */}
          <div className="p-8">
            {/* Título igual al header del onboarding */}
            ...
            <form onSubmit={handleEmailLogin} className="flex flex-col gap-4">
              ...
              <div className="flex gap-3 pt-4 border-t border-[#E2E8F0] mt-2">
                <motion.button
                  ...
                  type="submit"
                  disabled={isLoading || isGoogleLoading}
                  className="flex-1 bg-[#1A1A1A] text-white font-medium py-3 text-sm rounded-lg hover:bg-[#333] transition-colors disabled:opacity-50 shadow-sm flex items-center justify-center gap-2"
                >
                  {isLoading ? (
                    <>
          ...
          {/* Footer igual al del onboarding */}
          ...
          <div className="px-8 py-4 flex items-center justify-between bg-[#FBFBFA]">
            ...
            <span className="text-[10px] font-semibold text-[#94A3B8]">Setup 2.0</span>
          </div>
        </motion.div>
      </div>
    </div>
